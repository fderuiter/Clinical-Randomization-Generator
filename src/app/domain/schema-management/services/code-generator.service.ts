...
export class CodeGeneratorService {
   ...
   */
  generate(language: 'R' | 'SAS' | 'Python' | 'STATA', config: RandomizationConfig, validationSample: ValidationSampleItem[] = []): string {
    this.validateConfig(config);
    let code = '';
    if (config.randomizationMethod === 'MINIMIZATION') {
      switch (language) {
        case 'R':      code = this.buildRMinimization(config); break;
        case 'SAS':    code = this.buildSasMinimization(config); break;
        case 'Python': code = this.buildPythonMinimization(config); break;
        case 'STATA':  code = this.buildStataMinimization(config); break;
        default:       throw new UnsupportedLanguageError(language as string, config);
      }
    } else {
      switch (language) {
        case 'R':      code = this.generateR(config); break;
        case 'SAS':    code = this.generateSas(config); break;
        case 'Python': code = this.generatePython(config); break;
        case 'STATA':  code = this.generateStata(config); break;
        default:       throw new UnsupportedLanguageError(language as string, config);
      }
    }

    const header = this.buildValidationHeader(language, validationSample);
    const parityCheck = this.buildParityCheckBlock(language, validationSample);

    // Inject the validation header block near the top, after the first few lines of comments
    if (header) {
      if (language === 'SAS') {
        code = code.replace(/(\/\* Generated At:.*?\*\/)/, `$1\n${header}`);
      } else {
        const prefix = language === 'STATA' ? '*' : '#';
        // Note: some templates have "Generated At:" while others don't, but all have "Generated by" or similar except maybe SAS MINIMIZATION. Let's just prepend it before the PRNG Algorithm line, or if not found, right before set seed.
        code = code.replace(new RegExp(`\\n?\\s*${prefix === '*' ? '\\*' : prefix} PRNG Algorithm:`), `\n${header}\n${prefix} PRNG Algorithm:`);
        if (!code.includes(header)) {
          // fallback: insert after the first comment block
          code = code.replace(/(\n\s*(?:set\.seed|rng = np\.random\.default_rng|%let seed|set seed))/, `\n${header}$1`);
        }
      }
    }
  ...
  private buildValidationHeader(language: 'R' | 'SAS' | 'Python' | 'STATA', sample: ValidationSampleItem[]): string {
    if (!sample || sample.length === 0) return '';
    const prefix = language === 'SAS' ? '/*' : (language === 'STATA' ? '*' : '#');
    const suffix = language === 'SAS' ? ' */' : '';
    const lines = [
      `${prefix} --- Validation Sample (First ${sample.length} Subjects) ---${suffix}`,
      `${prefix} The following sequence was generated by the browser's PRNG.${suffix}`,
      `${prefix} The injected parity check will compare this against your local output.${suffix}`
    ];
    sample.forEach((s, i) => {
      lines.push(`${prefix} ${i + 1}. ${s.subjectId}: ${s.treatment}${suffix}`);
    });
    return lines.join('\n');
  }

  private buildParityCheckBlock(language: 'R' | 'SAS' | 'Python' | 'STATA', sample: ValidationSampleItem[]): string {
    if (!sample || sample.length === 0) return '';
    
    if (language === 'R') {
      const expSub = sample.map(s => `"${this.escapeRString(s.subjectId)}"`).join(', ');
      const expTrt = sample.map(s => `"${this.escapeRString(s.treatment)}"`).join(', ');
      return `
# --- Parity Check ---
expected_subjects <- c(${expSub})
expected_treatments <- c(${expTrt})

if (exists("schema") && nrow(schema) > 0) {
  check_len <- min(nrow(schema), length(expected_subjects))
  mismatch <- FALSE
  for (i in 1:check_len) {
    if (schema$SubjectID[i] != expected_subjects[i] || schema$Treatment[i] != expected_treatments[i]) {
      mismatch <- TRUE
      break
    }
  }
  if (mismatch) {
    warning("Parity Check Failed: The generated sequence diverges from the browser-based validation sample. This is expected due to native PRNG algorithm differences. Refer to the web UI for the golden sequence.")
  } else {
    message("Parity Verified: The local randomization sequence matches the web validation sample.")
  }
}
`;
    } else if (language === 'Python') {
      const expSub = sample.map(s => `"${this.escapePythonString(s.subjectId)}"`).join(', ');
      const expTrt = sample.map(s => `"${this.escapePythonString(s.treatment)}"`).join(', ');
      return `
# --- Parity Check ---
expected_subjects = [${expSub}]
expected_treatments = [${expTrt}]

if 'df' in locals() and not df.empty:
    mismatch = False
    check_len = min(len(df), len(expected_subjects))
    for i in range(check_len):
        if df.iloc[i]["SubjectID"] != expected_subjects[i] or df.iloc[i]["Treatment"] != expected_treatments[i]:
            mismatch = True
            break

    if mismatch:
        import warnings
        warnings.warn("Parity Check Failed: The generated sequence diverges from the browser-based validation sample. This is expected due to native PRNG algorithm differences. Refer to the web UI for the golden sequence.")
    else:
        print("Parity Verified: The local randomization sequence matches the web validation sample.")
`;
    } else if (language === 'SAS') {
      const expSub = sample.map(s => `'${this.escapeSasString(s.subjectId)}'`).join(' ');
      const expTrt = sample.map(s => `'${this.escapeSasString(s.treatment)}'`).join(' ');
      return `
/* --- Parity Check --- */
data _null_;
  /* Detect if final dataset is final_schema or _schema_minimization or _schema_marginal */
  if exist('final_schema') then dsid = open('final_schema');
  else if exist('_schema_minimization') then dsid = open('_schema_minimization');
  else if exist('_schema_marginal') then dsid = open('_schema_marginal');
  else return;

  array exp_sub[${sample.length}] $50 _temporary_ (${expSub});
  array exp_trt[${sample.length}] $50 _temporary_ (${expTrt});
  
  mismatch = 0;
  do i = 1 to ${sample.length};
    rc = fetchobs(dsid, i);
    if rc = 0 then do;
      sub_id = getvarc(dsid, varnum(dsid, 'SubjectID'));
      trt = getvarc(dsid, varnum(dsid, 'Treatment'));
      if trim(sub_id) ^= trim(exp_sub[i]) or trim(trt) ^= trim(exp_trt[i]) then mismatch = 1;
    end;
    else leave;
  end;
  rc = close(dsid);
  
  if mismatch = 1 then put 'WARNING: Parity Check Failed: The generated sequence diverges from the browser-based validation sample. This is expected due to native PRNG algorithm differences. Refer to the web UI for the golden sequence.';
  else put 'NOTE: Parity Verified: The local randomization sequence matches the web validation sample.';
run;
`;
    } else if (language === 'STATA') {
      const macros = sample.map((s, i) => `local exp_sub_${i+1} "${this.escapeStataString(s.subjectId)}"\nlocal exp_trt_${i+1} "${this.escapeStataString(s.treatment)}"`).join('\n');
      return `
* --- Parity Check ---
${macros}

local mismatch = 0
local check_len = min(_N, ${sample.length})
if \`check_len' > 0 {
    forvalues i = 1/\`check_len' {
        if SubjectID[\`i'] != "\`exp_sub_\`i''" | Treatment[\`i'] != "\`exp_trt_\`i''" {
            local mismatch = 1
        }
    }
    if \`mismatch' == 1 {
        display as error "WARNING: Parity Check Failed: The generated sequence diverges from the browser-based validation sample. This is expected due to native PRNG algorithm differences. Refer to the web UI for the golden sequence."
    }
    else {
        display as result "Parity Verified: The local randomization sequence matches the web validation sample."
    }
}
`;
    }
  ...
  private escapeStataString(s: string): string {
    return s.replace(/"/g, '""');
  }
