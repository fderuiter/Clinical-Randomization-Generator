...
export class DataExportService {
  ...
  exportCsv(data: RandomizationResult, isUnblinded: boolean): void {
    if (!data || !data.schema.length) return;
    ...
    link.setAttribute('download', `randomization_${data.metadata.protocolId}_${isUnblinded ? 'unblinded' : 'blinded'}.csv`);
  ...
  exportPdf(data: RandomizationResult, isUnblinded: boolean): void {
    if (!data || !data.schema.length) return;
    ...
    doc.text(`Status: ${isUnblinded ? 'UNBLINDED' : 'BLINDED'}`, 14, 66);
    ...
    doc.save(`randomization_${data.metadata.protocolId}_${isUnblinded ? 'unblinded' : 'blinded'}.pdf`);
  }
}
