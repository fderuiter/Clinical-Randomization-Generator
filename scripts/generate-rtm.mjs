...
lines.push('');
lines.push('The Equipose project enforces continuous cryptographic and static verification of the supply chain:');
lines.push('');
lines.push('* **OpenSSF Scorecard:** Monitored continuously in the `scorecard.yml` workflow.');
lines.push('* **Commit Signatures:** All commits to the main branch require valid cryptographic signatures (verified via CI).');
lines.push('* **OIDC Deployment:** Secrets have been replaced with OIDC-based identity providers for zero-trust deployment.');
lines.push('* **Signed Build Artifacts:** All generated release artifacts (including the SBOM) are securely archived via GitHub Actions.');
lines.push('');
lines.push('For real-time integrity metrics, visit the **Trust Center** in the application interface.');
lines.push('');
lines.push('---');
lines.push('');
